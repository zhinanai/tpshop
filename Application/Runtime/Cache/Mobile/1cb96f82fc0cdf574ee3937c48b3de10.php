	...
	<div class="order ajax_return">
	   <?php if(is_array($lists)): $i = 0; $__LIST__ = $lists;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$list): $mod = ($i % 2 );++$i;?><div class="order_list">
          <h2>
         	...
         	<a href="<?php echo U('/Mobile/User/order_detail',array('id'=>$list['order_id']));?>">
	          <?php if(is_array($list["goods_list"])): $i = 0; $__LIST__ = $list["goods_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$good): $mod = ($i % 2 );++$i;?><dl>  
				  ...
				  <dd class="pice">                  
                  	<em>
                    	  <?php if($list["return_btn"] == 1): ?><a href="<?php echo U('Mobile/User/return_goods',array('order_id'=>$list[order_id],'order_sn'=>$list[order_sn],'goods_id'=>$good[goods_id],'spec_key'=>$good['spec_key']));?>" style="color:#999;">申请售后</a><?php endif; ?>
                    </em>
                  </dd>                    
          ...
          <div class="anniu" style="width:95%">
                ...
                <?php if($list["return_btn"] == 1): ?><a href="mqqwpa://im/chat?chat_type=wpa&uin=<?php echo ($storeList[$list['store_id']]['store_qq']); ?>&version=1&src_type=web&web_src=oicqzone.com" target="_blank">联系客服</a><?php endif; ?>                        
          </div>
