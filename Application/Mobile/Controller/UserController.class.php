...
class UserController extends MobileBaseController
{
    ...
    */
    public function _initialize()
    {
        ...
        if (session('?user')) {
            $user = session('user');
            $user =$this->getUser()->vilidateUserById($user['user_id']);
            $user[0]['user_id']=$user[0]['userid'];
            $user[0]['nickname']=$user[0]['username'];
            $user[0]['user_money']=$user[0]['cangku_num'];
            $user[0]['pay_points']=$user[0]['fengmi_num'];
            session('user', $user[0]);  //覆盖session 中的 user
            $this->user = $user[0];
            $this->user_id = $user[0]['userid'];
    ...
    public function account()
    {
        ...
        $data = $this->getUser()->get_account_log($this->user_id, I('get.type'));
    ...
    public function login()
    {
        ...
        $code=I("code");//获取用户授权code
        if(!empty($code)){
            //执行解密
           $userid=$this->encrypt(urldecode($code),"D","YIWU");

            if($res=$this->getUser()->vilidateUserById($userid)){

                //用户存在
          /*      $res['url'] = urldecode(I('post.referurl'));*/
                $res[0]['user_id']= $res[0]['userid'];


                session('user', $res[0]);
                setcookie('user_id', $res[0]['userid'], null, '/');
                setcookie('is_distribut', 0, null, '/');
                $nickname = $res[0]['username'];
                setcookie('uname', $nickname, null, '/');
                setcookie('cn',0,time()-3600,'/');
                //$cartLogic = new \Home\Logic\CartLogic();
                //$cartLogic->login_cart_handle($this->session_id, $res[0]['userid']);  //用户登录后 需要对购物车 一些操作
...
//                    $this->error($check_code['msg']);
    ...
    */
    ...
    /**
     * 确认收货买家付款金额百分之80转换成积分返回给买家
     * 确认收货买家付款金额百分之80转换成余额百分之20转换成积分给卖家
     * @param $id int 订单id
     * */
   public function payOk($id){
        $orderInfo=M("order")->where("order_id=$id")->find();

        if($orderInfo['order_status'] != 1)
            return ;
        if($orderInfo){
            $getUserId=$orderInfo['user_id'];//买家id
            $sellerId=$orderInfo['store_id'];//卖家id

           $this->getUser()->updateUserJifen($getUserId,($orderInfo['order_amount']*0.8),43,2);//给买家+百分之80积分
            $this->getUser()->updateUserJifen($getUserId,($orderInfo['order_amount']*0.8),41,1);//给卖家+百分之80余额
            $this->getUser()->updateUserJifen($getUserId,($orderInfo['order_amount']*0.2),42,2);//给卖家+百分之20积分
        }
    }
