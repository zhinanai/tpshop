  ...
  /*      $pay_radio = $_REQUEST['pay_radio'];
    ...
    public function getCode(){     
            ...
            if($order['pay_status'] == 1){
            	$this->error('此订单，已完成支付!',"/index.php/Mobile/User/order_list/type/WAITSEND.html");
            }else{
                //进行支付操作
                $userInfo=$this->getUser()->vilidateUserById(session("user.user_id"));
                if($userInfo[0]['cangku_num']>$order['order_amount']){
                      //执行买家扣款并写入明细
                    $this->getUser()->updateUserAmount(session("user.user_id"),$order['order_amount'],39,$order['order_id'],"-")?
                    $this->success("付款成功等待卖家发货","/index.php/Mobile/User/order_list/type/WAITSEND.html") : $this->error("付款有误");
                }else{
                    $this->error("余额不足");
                }
            }
